"""
Мэтчи и сообщения: список мэтчей, история, отправка.
Роутинг: ?action=list|messages|send&match_id=...
"""
import json
import os
import psycopg2
    ...
    try:
        ...
        if action == 'list':
            cur.execute("""
                ...
                ORDER BY COALESCE((SELECT created_at FROM spark_messages WHERE match_id = m.id ORDER BY created_at DESC LIMIT 1), m.created_at) DESC
            ...
            for r in rows:
                ...
                if p:
                    result.append({
                        ...
                        'name': p[0], 'photo': (p[1] or [''])[0], 'age': p[2], 'verified': p[3],
                        'matched_at': matched_at.isoformat(),
                        'last_message': last_msg, 'last_time': last_time.isoformat() if last_time else None,
                        'unread': int(unread), 'online': True,
                    })
        ...
        if action == 'messages':
            ...
            cur.execute("SELECT id, sender_id, text, read, created_at FROM spark_messages WHERE match_id = %s ORDER BY created_at ASC", (match_id,))
            msgs = cur.fetchall()
            ...
            return {'statusCode': 200, 'headers': CORS, 'body': json.dumps({'messages': [
                {'id': str(m[0]), 'sender_id': str(m[1]), 'text': m[2], 'read': m[3], 'created_at': m[4].isoformat()} for m in msgs
            ]})}
        ...
        if action == 'send' and method == 'POST':
            ...
            if not cur.fetchone():
                return {'statusCode': 403, 'headers': CORS, 'body': json.dumps({'error': 'Нет доступа'})}
            cur.execute("INSERT INTO spark_messages (match_id, sender_id, text) VALUES (%s, %s, %s) RETURNING id, created_at", (match_id, user_id, text))
            row = cur.fetchone()
            conn.commit()
            return {'statusCode': 200, 'headers': CORS, 'body': json.dumps({'id': str(row[0]), 'sender_id': str(user_id), 'text': text, 'created_at': row[1].isoformat(), 'read': False})}
